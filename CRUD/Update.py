...
def update_admin(id: int, admin_update: Admin, user=Depends(get_user_role)):
    ...
    with Session(engine) as session:
        ...
        admin.sqlmodel_update(admin_update.model_dump(exclude={"password"}))
        session.commit()
...
def update_student(id: int, student_update: Student, user=Depends(get_user_role)):
    ...
    with Session(engine) as session:
        ...
        student.sqlmodel_update(student_update.model_dump(exclude={"password"}))
        session.commit()
...
def update_teacher(id: int, teacher_update: Teacher, user=Depends(get_user_role)):
    ...
    with Session(engine) as session:
        ...
        teacher.sqlmodel_update(teacher_update.model_dump(exclude={"password"}))
        session.commit()
...
def update_class(id: int, class_update: Class, user=Depends(get_user_role)):
    ...
    with Session(engine) as session:
        ...
        cls.sqlmodel_update(class_update.model_dump())
        session.commit()
        session.refresh(cls)
...
def update_subject(id: int, subject_update: Subject, user=Depends(get_user_role)):
    ...
    with Session(engine) as session:
        ...
        subject.sqlmodel_update(subject_update.model_dump())
        session.commit()
...
def update_grade(id: int, grade_update: Grade, user=Depends(get_user_role)):
    ...
    with Session(engine) as session:
        ...
        grade.sqlmodel_update(grade_update.model_dump())
        session.commit()
...
def update_class_teacher(id: int, ct_update: ClassTeacher, user=Depends(get_user_role)):
    ...
    with Session(engine) as session:
        ...
        ct.sqlmodel_update(ct_update.model_dump())
        session.commit()
        session.refresh(ct)
        return ct.model_dump()

@router.put("/ChangePassword/{name}/{password}")
def change_password(name: str, new_password: str, confirm_password: str, user=Depends(get_user_role)):
    if user["name"] != name and user["role"] != "admin":
    ...
    with Session(engine) as session:
        person = session.exec(select(Admin).where(Admin.name == name)).first()
        if not person:
            person = session.exec(select(Teacher).where(Teacher.name == name)).first()
        if not person:
            person = session.exec(select(Student).where(Student.name == name)).first()
        if not person:
